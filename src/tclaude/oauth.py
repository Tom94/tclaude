...
def is_expiring(token: dict[str, JSON]) -> bool:
    expires_at = get_or(token, "expires_at", 0.0)
    if expires_at < time.time() + 300:  # If we're closer than 5 minutes to expiry, refresh
        return True
    return False
...
class OAuth2Client:
    ...
    async def authenticate(self, session: aiohttp.ClientSession) -> dict[str, JSON]:
        ...
        try:
            async with session.post(token_url, data=body, headers=headers) as response:
                response.raise_for_status()
                token = client.parse_request_body_response(await response.text())
    ...
    async def refresh_token(self, session: aiohttp.ClientSession, token: dict[str, JSON]) -> dict[str, JSON]:
        ...
        try:
            async with session.post(token_url, data=body, headers=headers) as response:
                response.raise_for_status()
                new_token = client.parse_request_body_response(await response.text())
